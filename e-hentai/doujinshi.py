...
# from ehentai.constant import DETAIL_URL, IMAGE_URL
# from ehentai.logger import logger
# from ehentai.utils import format_filename
        ...
        if self.downloader:
            ...
            for i in range(1, min(self.pages, len(self.ext)) + 1):
                download_queue.append(
                    '%s/%d/%d.%s' % (IMAGE_URL, int(self.img_id), i, self.ext[i-1]))
...
if __name__ == '__main__':
    test = Doujinshi(name='test ehentai doujinshi', id=1)
    print(test)
    test.show()
    try:
        test.download()
